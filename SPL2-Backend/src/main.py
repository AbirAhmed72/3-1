...
async def delete_appointment(appointment_id: int, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    status = services.delete_appointment(db, appointment_id)
    if status:
        return {
            "status": "Appointment deleted successfully!"
        }
